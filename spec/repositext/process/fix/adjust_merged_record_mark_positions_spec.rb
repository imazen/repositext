...
class Repositext
  class Process
    class Fix
      describe AdjustMergedRecordMarkPositions do
        ...
        [
          ...
          [
            ...
            %(word word\n{: .normal_pn}\n\n\n^^^ {: .rid #rid-54250049 kpn="004"}\n*11*{: .pn} %word word),
            %(word word\n{: .normal_pn}\n\n^^^ {: .rid #rid-54250049 kpn="004"}\n\n*11*{: .pn} %word word),
          ],
        ].each do |(desc, txt, xpect)|
          it desc do
            o = AdjustMergedRecordMarkPositions.fix(txt, '_dummy_filename')
        ...
        describe '.distance_to_last_para_break' do
          [
            ...
            it "handles '#{ txt }'" do
              AdjustMergedRecordMarkPositions.distance_to_last_para_break(
                txt
              ).must_equal xpect
            end
          end
        ...
        describe '.distance_to_first_para_break' do
          [
            ...
            it "handles '#{ txt }'" do
              AdjustMergedRecordMarkPositions.distance_to_first_para_break(
                txt
              ).must_equal xpect
            end
          end
        ...
        describe '.make_sure_there_is_a_blank_line_after_first_record_mark' do
          [
            ["^^^\nsimple case", "^^^\n\nsimple case"],
            ["^^^ {: .rid #rid-59050019 kpn=\"001\"}\nwith ial", "^^^ {: .rid #rid-59050019 kpn=\"001\"}\n\nwith ial"],
          ].each do |(txt, xpect)|
            it "modifies '#{ txt }'" do
              AdjustMergedRecordMarkPositions.make_sure_there_is_a_blank_line_after_first_record_mark(
                txt
              ).must_equal xpect
            end
          end
