  ...
  class Entity
    class SubtitleMark
      ...
      protected
        ...
        # Adds spid to the inventory of spids to guarantee uniqueness. Keeps
        # SPIDs sorted alphabetically, one per line.
        # @param spid [Array<String>]
        def add_spids_to_inventory(spids)
          @inventory_file.rewind
          existing_spids = @inventory_file.read.split("\n").compact
          @inventory_file.rewind
          @inventory_file.write(
            (existing_spids + spids).uniq.sort.join("\n") + "\n"
          )
        ...
        def spid_exists_in_inventory_file?(spid)
          ...
          !!@spids_from_inventory_file.index("#{ spid }\n")
        end
