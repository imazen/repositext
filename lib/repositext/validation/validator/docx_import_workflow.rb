    ...
    class Validator
      ...
      protected
        ...
        def valid_docx_workflow?(docx_file_name)
          ...
          validate_correct_file_name(docx_file, errors, warnings)
        ...
        def validate_correct_file_name(docx_file, errors, warnings)
          if(
            docx_file.basename !~ /[[:alpha:]]{3}\d{2}-\d{4}[[:alpha:]]?_\d{4}\.docx/
          )
            ...
            raise ContentAtFileWithSubtitlesExistsError.new(
              [
                ...
                'It needs to look like this example: "eng56-0101_0297.docx"',
                "This is what it looked like instead: #{ docx_file.basename }",
                '',
                '',
              ].join("\n")
            )
