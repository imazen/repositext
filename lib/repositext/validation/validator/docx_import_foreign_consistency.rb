    ...
    class Validator
      ...
      private
        ...
        def find_inconsistent_plain_text(f_content_at_file, p_content_at_file, errors, warnings)
          ...
          content_at_plain_text = f_content_at_file.plain_text_with_subtitles_contents({})
        ...
        def find_inconsistent_eagles(f_content_at_file, p_content_at_file, errors, warnings)
          ...
          diffs.each { |e|
            ...
            error_detail = case e.first
            when '-'
              "Foreign is missing eagle at primary character position #{ (e.last * p_pt_len).round }."
            when '+'
              "Foreign has extra eagle at character position #{ (e.last * f_pt_len).round }."
            when '!'
              delta = (e[1] - e[2]).abs
              if delta >= (e[1] * tolerance_factor)
                "Foreign eagle character position #{ (e[2] * f_pt_len).round } is significantly different from primary position #{ (e[1] * p_pt_len).round }."
              else
                nil
              end
            else
